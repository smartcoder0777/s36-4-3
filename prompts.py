...
def build_system_prompt() -> str:
    return (
        ...
        "CRM UPDATE MATTER: Matters table → row matching ALL constraints → open editor (Revise/row click) → "
        "set Status → Save.\n"
        "JOBS (AutoConnect): Filter/search by job title + location, open listing, click Apply.\n"
        "JOB POST TITLE (AutoWork): Open Post a Job, type a title whose text contains the required substring "
        "(see job_title_must_contain in credentials).\n"
        "READING LIST (AutoBooks): After login, open the book detail page, then Add to reading list—not only search.\n"
        "DELIVERY MODAL: Use TASK_CREDENTIALS restaurant (and price hints); open restaurant → menu item → add-to-cart modal.\n"
        "RESERVE HOTEL: Use location + guests from credentials; search → listing → guests → Book → payment → Confirm.\n"
        ...
        "CREDENTIALS: username/email may have trailing spaces - type them exactly as shown in TASK_CREDENTIALS "
        "(user<web_agent_id> is already expanded to user{N} from the page URL).\n"
